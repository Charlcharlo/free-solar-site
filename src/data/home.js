export const placeholder = {
    title: "Lorem Ipsum",
    body: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec vitae elementum erat. Nunc vitae dui non lorem scelerisque lobortis. Etiam auctor a purus sagittis congue. Praesent ultricies sollicitudin sapien at iaculis. Aliquam erat volutpat. Nam laoreet pulvinar elit a aliquet. Maecenas blandit sem turpis, sed efficitur dolor ornare porttitor.",
    image: "./images/nhh.jpg"
}

export const aboutUs = {
    title: "On a mission to provide renewable energy for all",
    body: "FreeSolarZA was founded in 2021 with the goal of getting our country tapped into the sun and as off the grid as possible. After researching the best way to provide renewable energy to his home, our founder, Ivan Minev, soon saw he could use his findings and expertise to help many more people do the same. To date, FreeSolarZA has helped homes and businesses alike move towards a more sustainable future by overseeing the construction and maintenance of solar wifi hotspots, off-the-grid security cameras, as well as full-scale solar conversions so that you never have to worry about grid instability again.",
    image: "./images/solar-installation.jpg"
}

export const whySolar = {
    title: "Why choose solar power?",
    reasons: [
        {
            text: "Return on Investment",
        },
        {
            text: "Eliminate Electricity Bills"
        },
        {
            text: "Energy Independence"
        },
        {
            text: "Renewable Energy Source"
        }
    ]
}

export const specialProducts = [
    {
        title: "Solar Wifi Tower",
        icon: "./images/icons/wifi-tethering-rounded.svg",
        features: [
            "Off the grid wifi",
            "low maintenance",
            "range of up to 20km"
        ]
    },
    {
        title: "Off-grid Cameras",
        icon: "./images/icons/camera-video-outline.svg",
        features: [
            "Live monitoring",
            "Remote Storage",
            "Solar powered"
        ]
    },
    {
        title: "Self-timing lamps",
        icon: "./images/icons/street-light.svg",
        features: [
            "Low-maintenance",
            "Remote controlled",
            "Ideal for gardens, driveways and parks"
        ]
    }
]

export const panelOptions = [
    {
        title: "Small",
        icon: "./images/icons/solar-energy.svg",
        features: [
            "Wattage",
            "Ideal for the essentials"
        ]
    },
    {
        title: "Medium",
        icon: "./images/icons/solar-energy.svg",
        features: [
            "Wattage",
            "Ideal for the apartments"
        ]
    },
    {
        title: "Large",
        icon: "./images/icons/solar-energy.svg",
        features: [
            "Wattage",
            "Full-scale solution"
        ]
    }
]

export const contactLinks = [
    {
        text: "example@freesolar.com",
        icon: "email"
    },
    {
        text: "xxx-xxx-xxxx",
        icon: "phone"
    },
    {
        text: "Facebook",
        icon: "facebook"
    }
]